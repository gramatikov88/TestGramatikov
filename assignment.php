...
$now = time();
$window_ok = true;
if (!empty($assignment['open_at']) && strtotime($assignment['open_at']) > $now)
    $window_ok = false;
if (!empty($assignment['close_at']) && strtotime($assignment['close_at']) < $now)
    $window_ok = false;
if (!$assignment['is_published'] || !$window_ok) {
...
if (!$ok) {
    http_response_code(403);
    die('Нямате достъп до това задание.');
}
...
<body>
    ...
    <?php $testTheme = $assignment['theme'] ?? 'default';
    $testThemeConfig = $assignment['theme_config'] ?? null;
    include __DIR__ . '/components/test_styles.php'; ?>
    ...
    <main class="container my-4 my-md-5">
            ...
            <div class="col-md-8">
                <div class="alert alert-info py-2 mb-0">
                    ...
                    <?php if ((int) $assignment['attempt_limit'] > 0): ?>
                        <span class="ms-3">Опит: <strong><?= $prev_attempts ?></strong> от
                            <strong><?= (int) $assignment['attempt_limit'] ?></strong></span>
                    <?php else: ?>
                    ...
                    <?php if ($strict_mode_active): ?>
                        <span class="ms-3 text-danger fw-semibold">Стриктен режим: напускане на страницата анулира
                            опита</span>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <form method="post" id="assignmentForm">
            ...
            <?php if ($strict_mode_active): ?>
                <div id="strictModeNotice" class="alert alert-danger d-none">Нарушихте строгия режим. Опитът ще бъде
                    анулиран и оценката се фиксира на 2.</div>
            <?php endif; ?>
            <?php foreach ($questions as $idx => $q): ?>
                <div class="card shadow-sm mb-3 q-card" data-question-id="<?= (int) $q['id'] ?>"
                    data-question-type="<?= htmlspecialchars($q['qtype']) ?>">
                    <div class="card-body">
                        ...
                        <div class="mt-2">
                            <?php if (in_array($q['qtype'], ['single_choice', 'true_false'], true)): ?>
                                <?php foreach ($q['answers'] as $a): ?>
                                    <div class="form-check">
                                        <input class="form-check-input" type="radio" name="q_<?= (int) $q['id'] ?>"
                                            value="<?= (int) $a['id'] ?>" id="a<?= (int) $a['id'] ?>" />
                                        <label class="form-check-label"
                                            for="a<?= (int) $a['id'] ?>"><?= htmlspecialchars($a['content']) ?></label>
                                    </div>
                                <?php endforeach; ?>
                            <?php elseif ($q['qtype'] === 'multiple_choice'): ?>
                                <?php foreach ($q['answers'] as $a): ?>
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="q_<?= (int) $q['id'] ?>[]"
                                            value="<?= (int) $a['id'] ?>" id="a<?= (int) $a['id'] ?>" />
                                        <label class="form-check-label"
                                            for="a<?= (int) $a['id'] ?>"><?= htmlspecialchars($a['content']) ?></label>
                                    </div>
                            ...
                            <?php elseif ($q['qtype'] === 'short_answer'): ?>
                                <input type="text" name="q_<?= (int) $q['id'] ?>" class="form-control"
                                    placeholder="Вашият отговор" />
                            <?php elseif ($q['qtype'] === 'numeric'): ?>
                                <input type="number" step="any" name="q_<?= (int) $q['id'] ?>" class="form-control"
                                    placeholder="Число" />
                            <?php endif; ?>
                            <input type="hidden" name="qs_time[<?= (int) $q['id'] ?>]" class="question-time-input"
                                data-question-id="<?= (int) $q['id'] ?>" value="0" />
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
            ...
            <div class="d-flex justify-content-end">
                <button type="submit" class="btn btn-primary" <?= $can_attempt ? '' : 'disabled' ?>><i
                        class="bi bi-check2-circle me-1"></i>Предай</button>
            </div>
        </form>
    </main>
        ...
        <script>
            (function () {
                ...
                document.addEventListener('copy', function () {
                    logger.send('copy_attempt', { timestamp: Date.now() });
                });
                document.addEventListener('paste', function () {
                    logger.send('paste_attempt', { timestamp: Date.now() });
                });

                // ── NEW: Mouse leaving the document (potential alt-tab or cheat sheet) ──
                let mouseLeaveCount = 0;
                document.addEventListener('mouseleave', function (e) {
                    if (e.clientY <= 0 || e.clientX <= 0 || e.clientX >= window.innerWidth || e.clientY >= window.innerHeight) {
                        mouseLeaveCount++;
                        logger.send('mouse_leave', { count: mouseLeaveCount, timestamp: Date.now() });
                    }
                });

                // ── NEW: Right-click / context menu ──────────────────────────────
                document.addEventListener('contextmenu', function () {
                    logger.send('context_menu', { timestamp: Date.now() });
                });

                // ── NEW: Suspicious keyboard shortcuts ────────────────────────────
                const suspiciousKeys = new Set(['c', 'v', 'a', 'f', 'p', 'u', 's']);
                document.addEventListener('keydown', function (e) {
                    const key = e.key ? e.key.toLowerCase() : '';
                    // Ctrl/Cmd combos
                    if ((e.ctrlKey || e.metaKey) && suspiciousKeys.has(key)) {
                        logger.send('keyboard_shortcut', { combo: (e.ctrlKey ? 'Ctrl' : 'Meta') + '+' + key.toUpperCase(), timestamp: Date.now() });
                        return;
                    }
                    // F12 (DevTools), PrintScreen
                    if (e.key === 'F12' || e.key === 'PrintScreen') {
                        logger.send('keyboard_shortcut', { combo: e.key, timestamp: Date.now() });
                    }
                });

                // ── NEW: Screen info (once on load) ──────────────────────────────
