...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        try {
            ...
            // Update test
            $stmt = $pdo->prepare('UPDATE tests SET subject_id=:subject_id, title=:title, description=:description, visibility=:visibility, status=:status, time_limit_sec=:time_limit_sec, is_randomized=:is_randomized, theme=:theme, theme_config=:theme_config WHERE id=:id');
            $stmt->execute([
