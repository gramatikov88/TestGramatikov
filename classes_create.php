...
<head>
    ...
    <style>.scroll-area{max-height:220px;overflow:auto;border:1px solid #eee;border-radius:.5rem;padding:.5rem}</style>
</head>
<body>
    ...
    <form method="post" class="card shadow-sm mb-4">
        ...
        <div class="card-body row g-3">
            ...
            <div class="col-md-3">
                ...
                <input type="datetime-local" name="open_at" class="form-control" value="<?= $form['open_at'] ? str_replace(' ', 'T', $form['open_at']) : '' ?>" />
            </div>
            <div class="col-md-3">
                <label class="form-label">Срок</label>
                <input type="datetime-local" name="due_at" class="form-control" value="<?= $form['due_at'] ? str_replace(' ', 'T', $form['due_at']) : '' ?>" />
            </div>
            <div class="col-md-3">
                <label class="form-label">До</label>
                <input type="datetime-local" name="close_at" class="form-control" value="<?= $form['close_at'] ? str_replace(' ', 'T', $form['close_at']) : '' ?>" />
            </div>
            <div class="col-md-3">
                <label class="form-label">Лимит опити</label>
                <input type="number" min="0" name="attempt_limit" class="form-control" value="<?= (int) $form['attempt_limit'] ?>" />
            </div>
            <div class="col-md-3 d-flex align-items-end">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" id="shuffle_questions" name="shuffle_questions" <?= $form['shuffle_questions'] ? 'checked' : '' ?> />
                    <label class="form-check-label" for="shuffle_questions">Разбъркване на въпроси</label>
                </div>
            </div>
            <div class="col-md-3 d-flex align-items-end">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" id="is_published" name="is_published" <?= $form['is_published'] ? 'checked' : '' ?> />
        ...
        <div class="card-body">
            <div class="scroll-area">
                <?php foreach ($classes as $c):
                    $isChecked = in_array((int) $c['id'], $form['class_ids'], true); ?>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="class_ids[]" value="<?= (int) $c['id'] ?>" id="class<?= (int) $c['id'] ?>" <?= $isChecked ? 'checked' : '' ?> />
                            <label class="form-check-label" for="class<?= (int) $c['id'] ?>">
                                <?= htmlspecialchars($c['grade'] . $c['section']) ?> • <?= htmlspecialchars($c['school_year']) ?> — <?= htmlspecialchars($c['name']) ?>
                            </label>
                        </div>
                <?php endforeach; ?>
            </div>
            ...
            <div class="scroll-area">
                <?php foreach ($students as $s):
                    $isChecked = in_array((int) $s['id'], $form['student_ids'], true); ?>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="student_ids[]" value="<?= (int) $s['id'] ?>" id="st<?= (int) $s['id'] ?>" <?= $isChecked ? 'checked' : '' ?> />
                ...
                <?php endforeach; ?>
                <?php if (!$students): ?>
                        <div class="text-muted small">Няма намерени ученици във ваши класове.</div>
                <?php endif; ?>
            </div>
        </div>
