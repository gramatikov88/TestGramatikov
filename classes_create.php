...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['__action'] ?? '') === 'save_class') {
    ...
    if (!$errors) {
        try {
            ...
            // Ако имаш UNIQUE(teacher_id, grade, section, school_year) – 23000 = duplicate key
            if ($e->getCode() === '23000') {
...
if ($editing && $_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['__action'] ?? '') === 'add_student') {
    ...
    if (!$errors) {
        try {
            ...
            // Има ли потребител с този имейл?
            ...
            if ($existing) {
                ...
                // Създай нов ученик с временна парола
                $pwd = random_password();
                $created_password = $pwd;
                $hash = password_hash($pwd, PASSWORD_DEFAULT);

                // С колоната status (ако съществува в схемата) – задаваме active.
                // Ако нямаш поле status, закоментирай го.
                $stmt = $pdo->prepare('
...
<main class="container my-4 my-md-5">
    ...
    <?php if ($created_password): ?>
            <div class="alert alert-info">Създаден е нов ученик. Временна парола: <strong><?= htmlspecialchars($created_password) ?></strong></div>
    <?php endif; ?>
