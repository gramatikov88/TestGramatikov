...
function slugify(string $text): string
{
    ...
    $text = trim($text, '-');
    if ($text === '')
        $text = 'subject-' . substr(sha1(uniqid('', true)), 0, 6);
    return $text;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name === '')
        $errors[] = 'Моля, въведете име на предмета.';
    if ($slug === '')
        $slug = slugify($name);
    ...
    if (!$errors) {
        try {
            ...
            if ($e->getCode() === '23000') {
                $errors[] = 'Съществува предмет със същия slug.';
            } else {
...
<body>
    ...
    <main class="container my-4 my-md-5">
        ...
        <?php if ($saved): ?>
            <div class="alert alert-success">Запазено успешно.</div><?php endif; ?>
        <?php if ($errors): ?>
            ...
            <div class="col-lg-5">
                <div class="card shadow-sm h-100">
                    <div class="card-header bg-white">
                        <strong><?= $edit ? 'Редакция на предмет' : 'Нов предмет' ?></strong></div>
                    <form method="post">
                        <?= csrf_field() ?>
                        <div class="card-body">
                            <div class="mb-3">
                        ...
                        <div class="card-footer bg-white d-flex justify-content-end gap-2">
                            <?php if ($edit): ?><input type="hidden" name="id"
                                    value="<?= (int) $edit['id'] ?>" /><?php endif; ?>
                            <button class="btn btn-primary" type="submit"><i
                                    class="bi bi-check2-circle me-1"></i>Запази</button>
                            <?php if ($edit): ?><a class="btn btn-outline-secondary"
                                    href="subjects_create.php">Отказ</a><?php endif; ?>
                        </div>
                    </form>
                </div>
            </div>
            <div class="col-lg-7">
                <div class="card shadow-sm h-100">
                    ...
                    <div class="list-group list-group-flush">
                        <?php if (!$rows): ?>
                            <div class="list-group-item text-muted">Няма предмети.</div><?php endif; ?>
                            ...
                            <div class="list-group-item d-flex justify-content-between align-items-center">
                                ...
                                <div class="d-flex gap-2">
                                    <a class="btn btn-sm btn-outline-primary"
                                        href="subjects_create.php?id=<?= (int) $r['id'] ?>"><i class="bi bi-pencil"></i></a>
                                    <a class="btn btn-sm btn-outline-danger"
                                        href="subjects_create.php?delete=<?= (int) $r['id'] ?>"
                                        onclick="return confirm('Изтриване на предмета?');"><i class="bi bi-trash"></i></a>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    </div>
